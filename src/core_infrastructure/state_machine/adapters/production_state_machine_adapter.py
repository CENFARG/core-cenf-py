...
LoggerManager and RED metrics via ObservabilityManager. Supports both sync
and async handlers. All transitions are logged and measured.

Security: NEVER swallow errors — all handler exceptions are re-raised or
    recorded in status.errors before strategy application.
Observability: Emits cenf.state_machine.transitions_total counter,
    cenf.state_machine.errors_total counter, and duration histogram.
@ai-directive: NEVER swallow errors in production adapter. Every exception
    must be logged and measured before strategy application.
...
_COUNTER = "cenf.state_machine.transitions_total"
_ERROR_COUNTER = "cenf.state_machine.errors_total"
_DURATION = "cenf.state_machine.transition_duration_ms"


class ProductionStateMachineAdapter[StateT, ContextT]:
    """Production state machine with structured logging and metrics.

    Extends the in-memory pattern with LoggerManager and ObservabilityManager
    integration. Supports both sync and async handlers.

    Args:
        config: StateMachineConfig controlling initial state and behavior.
        logger_manager: LoggerManager for structured transition logs.
        observability_manager: ObservabilityManager for RED metrics.
    """
    ...
    def __init__(
        ...
        self._obs = observability_manager
    ...
    def register_handler(
        ...
        """Bind a handler to a state (sync or async)."""
    ...
    async def run(
        ...
        """Execute the state machine loop with logging and metrics."""
        ...
        self._logger.info("state_machine.started", initial_state=str(current))
        ...
        for _ in range(self._config.max_iterations):
            ...
            try:
                t0 = _time.monotonic()
                result = handler(ctx)
                next_state: Any = await result if inspect.isawaitable(result) else result
                elapsed = (_time.monotonic() - t0) * 1000
                self._obs.record_histogram(_DURATION, elapsed, {"state": str(current)})
            except Exception as exc:
                self._logger.error("state_machine.handler_error", exc=exc, state=str(current))
                self._obs.increment_counter(_ERROR_COUNTER, attributes={"state": str(current)})
            ...
            if not self.is_valid_transition(current, next_state):
                ...
                self._status.errors.append(f"Invalid transition: {current} -> {next_state}")
                break
            ...
            self._obs.increment_counter(_COUNTER, attributes={
        ...
        self._status.is_running = False
        self._status.is_terminated = True
        self._logger.info("state_machine.terminated", final_state=str(current))
        return self._status
